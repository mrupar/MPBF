...
def download_archive(
    ...
    endpoint = f"{API_BASE}/{dataset}/analysis/{pdb_chain}"
    ...
    try:
        if archive_type == 'metadata':
            ...
            # Only extract files that match the default suffix list (case-insensitive)
            suffixes = [s.lower() for s in DEFAULT_SUFFIXES]
            ...
            with zipfile.ZipFile(final_zip, 'r') as z:
                for member in z.namelist():
                    ...
                    if any(name_lower.endswith(s) for s in suffixes):
                        # Ensure target directory for this member exists and extract
                        z.extract(member, path=out_folder)
                        matched = True

            if not matched:
                logging.error(f"No files in archive {final_zip} matched suffixes {DEFAULT_SUFFIXES}")
